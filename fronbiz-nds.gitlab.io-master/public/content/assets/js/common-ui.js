...
function incHtml() {
    $('[data-includ]').each(function () {
        ...
        $this.load($url, function(res, status, xhr){
			console.log("end includ :",status);
			if(res.indexOf("일시적인 오류가 발생하였습니다") > -1 
				&& res.indexOf("type-error") > -1) {
				$this.empty();		
			}			
		});
    })
...
//     var inputElement = document.getElementById('tt1'); 
//     var wonElement = document.querySelector('.won'); 
    
//     inputElement.addEventListener('focus', function() { 
//         wonElement.classList.add('comb-del'); 
//     }); 
        
//     inputElement.addEventListener('blur', function() { 
//         wonElement.classList.remove('comb-del'); 
//     });
// });
    ...
    detailElements.forEach((detailElement) => {
        ...
        if (titleCheckbox) {
            titleCheckbox.addEventListener('change', function () {
                ...
                console.log('Title checkbox changed:', this.checked);
...
function morebtn() {
    $(document).on('click', '.popular-anchors .more', function(e){
        $('.popular-anchors').toggleClass('-active');
    })
}